...
class units {
	...
	function check_needed($unit,$village_info) {
		...
		if (empty($this->last_error)) {
			...
			$popadnicy = $pojemnosc_zagrody - $village_info['r_bh'];
			if ($this->get_bhprice($unit) > $popadnicy) {
		...
		if (empty($this->last_error)) {
			...
			$can_create['bh'] = floor($popadnicy / $this->get_bhprice($unit));
	...
	function recruit_units($unit,$count,$building,$build_stage,$vid) {
		...
		$vinfo = DB::Fetch(DB::Query("SELECT `bonus`,`uid` FROM `{$this->DB}`.`villages` WHERE `id` = '{$vid}'"));
		
		$recruit_time = $this->get_time($build_stage,$unit,$vinfo['bonus']) * $count;
		
		$last_recruit = DB::Fetch(DB::Query("SELECT `time_finished` FROM `{$this->DB}`.`recruit` WHERE `vid` = '{$vid}' AND `building` = '{$building}' ORDER BY `id` DESC LIMIT 1"))[0];
		...
		if (empty($last_recruit)) {
			...
			DB::Query("INSERT INTO `{$this->DB}`.`recruit`(`unit`,`num_unit`,`time_finished`,`time_start`,`time_per_unit`,`building`,`vid`,`uid`) VALUES('{$unit}','{$count}','{$time_end}','{$time_start}','{$time_per_unit}','{$building}','{$vid}','{$vinfo['uid']}')");
		} else {
	...
	function get_time($stage,$unit,$village_bonus = 0,$flag = 0) {
		global $bonus;
	...
	function read_units($from,$to) {
		...
		$sql = "SELECT `$implode_units` FROM `{$this->DB}`.`unit_place` WHERE ";
	...
	function max_unit_can_be_recruited($stone,$wood,$iron,$pop,$costs,$costd,$costr,$costb) {
		$ok[0] = floor($stone/$costs);
		$ok[1] = floor($wood/$costd);
		$ok[2] = floor($iron/$costr);
		$ok[3] = floor($pop/$costb);
	...
	function get_recruits_counts($vid,$unit) {
		$count = DB::Fetch(DB::Query("SELECT COUNT(id) FROM `{$this->DB}`.`recruit` WHERE `vid` = '{$vid}' AND `unit` = '{$unit}' LIMIT 1"))[0];
		if ($count > 0) {
			$ret = true;
		} else {
			$ret = false;
	...
	function subtract_resources($vid,$s,$w,$i,$pop) {
		mysql_query("UPDATE `{$this->DB}`.`villages` SET `stone_float` = `stone_float` - '{$s}' , `wood_float` = `wood_float` - '{$w}' , `iron_float` = `iron_float` - '{$i}' ,  `pop` = `pop` + '{$pop}' WHERE `id` = '{$vid}'");
		}
		
	function needs($assoc_array,$needs_array) {
	    $counter = 0;
	    foreach ($needs_array as $key => $val) {
		    if ($assoc_array[$key] >= $val) {
			    $counter += 1;
				}
			}
			
		if ($counter == count($needs_array)) {
		    $ret = true;
		} else {
			$ret = false;
		}
...
$cl_units->add_unit("Mnich","unit_mnich");
...
$cl_units->set_col("A");
$cl_units->set_attType("def");
$cl_units->set_description("Mnich jest jednostk¹ Obronn¹.Jest to najlepsza jednostka do obrony wioski - lecz te¿ i droga.");
}
